...
export default function Home({ toggleTheme, theme }) {
            ...
            featuresRef.current?.querySelectorAll('.reveal-up').forEach((el, i) => {
                ...
                gsap.fromTo(
                    el,
                    {
                        opacity: 0,
                        y: isHeading ? 30 : 70,
                        x: isHeading ? 0 : (i % 2 === 0 ? -40 : 40),
                        scale: isHeading ? 1 : 0.88,
                    },
                    {
    ...
    return (
        <div
            className={`relative min-h-screen transition-colors duration-300 ${dark ? 'bg-surface-950 text-slate-100' : 'bg-slate-50 text-slate-900'
                }`}
        >
