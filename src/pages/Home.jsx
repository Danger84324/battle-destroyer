        ...
        <div className={`relative min-h-screen transition-colors duration-300 ${dark ? 'bg-surface-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
                ...
                <div className="max-w-6xl mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
                    <div className="flex items-center gap-2 sm:gap-3">
                        <div className="relative">
                            ...
                            <img src="/logo512.png" alt="Battle Destroyer"
                                className="relative w-7 h-7 sm:w-8 sm:h-8 rounded-xl object-contain"
                                style={{ filter: 'drop-shadow(0 0 6px rgba(220,38,38,0.5))' }} />
                        </div>
                        <span className="text-red-500 tracking-[0.1em] sm:tracking-[0.15em] font-bold text-xs sm:text-base hidden xs:inline"
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-3">
                        <button onClick={toggleTheme}
                            className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${dark ? 'bg-white/[0.06] text-yellow-400' : 'bg-black/[0.05] text-slate-600'}`}>
                        ...
                        {isLoggedIn ? (
                            ...
                            <Link to="/login"
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${dark ? 'bg-white/[0.06] hover:bg-white/[0.1]' : 'bg-black/[0.04] hover:bg-black/[0.07]'}`}>
                                Login
                            </Link>
                        )}
                    </div>
            ...
            <section ref={heroRef} className="relative z-10 pt-8 sm:pt-20 pb-12 sm:pb-24 px-3 sm:px-6 text-center overflow-hidden">
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    <div className="reveal-up inline-flex items-center gap-1.5 sm:gap-3 px-2.5 sm:px-5 py-1 sm:py-2.5 rounded-full border mb-5 sm:mb-8 text-[9px] sm:text-sm font-bold uppercase tracking-wider"
                        style={{ background: 'linear-gradient(135deg, rgba(220,38,38,0.15), rgba(220,38,38,0.1))', borderColor: 'rgba(220,38,38,0.4)' }}>
                        <FaFire className="text-red-500 animate-pulse text-[9px] sm:text-sm" />
                        <span className="text-red-400 text-[8px] sm:text-sm">#1 BGMI DDoS PLATFORM • INDIA'S BEST</span>
                        <FaFire className="text-red-500 animate-pulse text-[9px] sm:text-sm" />
                    </div>

                    <h1 className="reveal-up" style={{ lineHeight: 1.1 }}>
                        <span className="block text-3xl sm:text-6xl md:text-7xl font-bold tracking-[0.04em] sm:tracking-[0.06em] mb-1 text-slate-200">
                            BATTLE
                        </span>
                        <span className="block text-4xl sm:text-7xl md:text-8xl font-bold tracking-[0.04em] sm:tracking-[0.06em] text-gradient-red"
                    ...
                    <div className="reveal-up flex flex-col sm:flex-row items-center justify-center gap-2.5 sm:gap-4 mt-5 sm:mt-8 md:mt-10">
                        {isLoggedIn ? (
                            <Link to="/dashboard"
                                ...
                                <FaBolt size={12} /> GO TO DASHBOARD
                            ...
                            <>
                                <Link to="/login"
                                    className="flex items-center gap-2 px-4 sm:px-8 py-2.5 sm:py-4 rounded-xl font-bold text-xs sm:text-base text-white bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 transition-all active:scale-95 group w-full sm:w-auto justify-center"
                                    style={{ boxShadow: '0 4px 20px rgba(220,38,38,0.4)' }}>
                                    <FaBullseye size={11} /> START ATTACK
                                    <FaArrowRight size={10} className="group-hover:translate-x-1 transition-transform" />
                                </Link>
                                <Link to="/reseller"
                                    className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-4 rounded-xl font-semibold text-xs sm:text-base transition-all w-full sm:w-auto justify-center ${dark ? 'bg-white/[0.06] hover:bg-white/[0.1] border border-white/[0.1]' : 'bg-black/[0.04] hover:bg-black/[0.07] border border-black/[0.08]'}`}>
                                    <FaGem size={12} /> Become Reseller
                                </Link>
                            </>
                        )}
                    </div>

                    <div className="reveal-up flex flex-wrap items-center justify-center gap-2 sm:gap-6 mt-5 sm:mt-8 md:mt-10">
                        {TRUST_BADGES.map((badge, idx) => (
                            <span key={idx} className={`flex items-center gap-1 text-[8px] sm:text-xs font-medium ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                                <badge.icon className={badge.color} size={10} />
                                {badge.text}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className={`rounded-2xl p-3 sm:p-6 md:p-8 border grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 ${dark ? 'bg-white/[0.03] border-white/[0.07]' : 'bg-black/[0.02] border-black/[0.07]'}`}>
                        {STATS.map((stat, i) => (
                            <div key={i} className={`reveal-up delay-${i + 1} text-center`}>
                                <stat.icon className="text-red-500/60 mx-auto mb-1.5" size={14} />
                                <p className="stat-value">{loading ? '...' : stat.value}</p>
                                <p className={`text-[8px] sm:text-xs mt-1 uppercase tracking-wider font-semibold ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
                                    {stat.label}
                                </p>
                            </div>
                        ))}
            ...
            <section ref={featuresRef} className="relative z-10 py-12 sm:py-20 px-3 sm:px-6">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-8 sm:mb-14">
                        ...
                        <h2 className="reveal-up text-xl sm:text-3xl md:text-5xl font-bold text-slate-800 dark:text-white">
                            India's <span className="text-gradient-red">#1 BGMI </span>DDoS Platform
                        </h2>
                        <p className={`reveal-up mt-2 sm:mt-4 text-xs sm:text-base ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
                        {FEATURES.map((f, i) => (
                            <div key={i}
                                className={`reveal-up delay-${(i % 3) + 1} group rounded-xl sm:rounded-2xl p-4 sm:p-6 border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${dark ? `bg-gradient-to-br ${f.bg} border-white/[0.07] hover:border-white/[0.15]` : `bg-white border-black/[0.07] hover:border-black/[0.12] shadow-sm`}`}>
                                <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-xl border bg-gradient-to-br ${f.bg} ${f.border} flex items-center justify-center mb-3 sm:mb-5`}>
                                    <f.icon className={f.color} size={18} />
                                </div>
                                <h3 className={`font-bold text-sm sm:text-lg mb-1.5 sm:mb-2 ${dark ? 'text-white' : 'text-slate-900'}`}>{f.title}</h3>
                                <p className={`text-[11px] sm:text-sm leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-500'}`}>{f.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        {STEPS.map((step, i) => (
                            <div key={i} className={`reveal-up delay-${i + 1} text-center group`}>
                                <div className="relative inline-block mb-3 sm:mb-5">
                                    <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl border-2 border-red-600/30 bg-red-600/10 flex items-center justify-center mx-auto group-hover:border-red-500/70 transition-all group-hover:scale-105"
                                        style={{ boxShadow: '0 0 20px rgba(220,38,38,0.1)' }}>
                                        <step.icon className="text-red-400" size={20} />
                                    </div>
                                    <span className="absolute -top-2 -right-2 w-5 h-5 sm:w-7 sm:h-7 rounded-full bg-gradient-to-br from-red-600 to-red-700 text-white text-[10px] sm:text-sm font-black flex items-center justify-center shadow-lg">
                                        {i + 1}
                                    </span>
                                </div>
                                <h3 className={`font-bold text-base sm:text-xl mb-1.5 sm:mb-2 ${dark ? 'text-white' : 'text-slate-900'}`}>{step.title}</h3>
                                <p className={`text-[11px] sm:text-sm leading-relaxed px-2 sm:px-4 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>{step.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section ref={trustRef} className="relative z-10 py-12 sm:py-16 px-3 sm:px-6">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-8 sm:mb-12">
                        ...
                        <h2 className="reveal-up text-xl sm:text-3xl md:text-4xl font-bold text-slate-800 dark:text-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                        {[
                            ...
                            <div key={i} className={`reveal-up delay-${i + 1} rounded-xl sm:rounded-2xl p-4 sm:p-6 border ${dark ? 'bg-white/[0.03] border-white/[0.07]' : 'bg-white border-black/[0.07] shadow-sm'}`}>
                                <div className="flex gap-0.5 mb-2.5">
                                    {[...Array(testimonial.rating)].map((_, j) => (
                                        <FaStar key={j} className="text-yellow-400 text-[10px] sm:text-sm" />
                                    ))}
                                </div>
                                <p className={`text-[11px] sm:text-sm italic mb-3 sm:mb-4 ${dark ? 'text-slate-300' : 'text-slate-600'}`}>"{testimonial.text}"</p>
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-r from-red-500 to-red-700 flex items-center justify-center text-white text-[10px] sm:text-xs font-bold">
                                        {testimonial.name[0]}
                                    </div>
                                    <div>
                                        <p className="text-[10px] sm:text-xs font-bold">{testimonial.name}</p>
            ...
            <section ref={ctaRef} className="relative z-10 py-12 sm:py-20 px-3 sm:px-6">
                <div className="max-w-5xl mx-auto">
                    <div className={`reveal-up rounded-2xl sm:rounded-3xl p-5 sm:p-10 md:p-14 border relative overflow-hidden ${dark ? 'bg-gradient-to-br from-red-600/10 via-red-900/20 to-surface-800 border-red-600/30' : 'bg-gradient-to-br from-red-50 via-red-50 to-white border-red-200'}`}>
                        ...
                        <FaFire className="absolute top-3 right-3 text-red-500/20 text-3xl sm:text-6xl" />
                        ...
                        <div className="relative z-10 text-center">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-2.5 sm:gap-4">
                                {isLoggedIn ? (
                                    <Link to="/reseller"
                                        className="flex items-center gap-2 px-5 sm:px-8 py-2.5 sm:py-4 rounded-xl font-bold text-xs sm:text-base text-white bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 transition-all w-full sm:w-auto justify-center"
                                        style={{ boxShadow: '0 4px 20px rgba(220,38,38,0.4)' }}>
                                        <FaGem size={12} /> RESELLER DASHBOARD
                                    </Link>
                                ) : (
                                    <Link to="/reseller-prices"
                                        className="flex items-center gap-2 px-5 sm:px-8 py-2.5 sm:py-4 rounded-xl font-bold text-xs sm:text-base text-white bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 transition-all w-full sm:w-auto justify-center"
                                        style={{ boxShadow: '0 4px 20px rgba(220,38,38,0.4)' }}>
                                        <FaGem size={12} /> JOIN & START EARNING
                                    </Link>
                                )}
                            </div>
                        </div>
