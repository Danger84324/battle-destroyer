...
export default function Dashboard({ toggleTheme, theme, setIsAuth }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`${API_URL}/api/panel/me`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setUser(data);
  ...
  /* ────────────────────────────────────────────────────────
     STAT CARDS
     Pro users  → NEVER show credits. Show subscription info.
     Free users → show credits, referrals, max duration.
  ──────────────────────────────────────────────────────── */
  const stats = user.isPro ? [
    ...
    },
    {
      ...
      unit: ' days',
      color: (user.subscriptionStatus?.daysLeft ?? 99) <= 3 ? 'text-red-400' : 'text-yellow-400',
      iconBg: 'bg-yellow-600/10 border-yellow-600/20',
      subtext: user.subscriptionStatus?.plan || 'Pro Plan'
    },
    {
      icon: FaKey,
      label: 'User ID',
  ...
  const howItWorks = user.isPro ? [
    {
      ...
      desc: '30 attacks per day with 300s max duration — no credits needed',
      color: 'text-purple-400', bg: 'bg-purple-600/10 border-purple-600/20'
    },
    {
      icon: FaLink,
      title: 'Refer Friends',
      desc: 'Share your referral link — earn bonus credits for every successful signup',
      color: 'text-green-400', bg: 'bg-green-600/10 border-green-600/20'
    },
    {
      icon: FaBullseye,
      title: 'Launch Attacks',
      desc: 'Enter IP, port, duration. Pass CAPTCHA and fire when ready',
      color: 'text-red-400', bg: 'bg-red-600/10 border-red-600/20'
    },
  ] : [
    {
      icon: FaGift,
      title: 'Signup Bonus',
      desc: 'Get 10 free credits when you create your account',
      color: 'text-green-400', bg: 'bg-green-600/10 border-green-600/20'
    },
    {
      icon: FaLink,
      title: 'Refer Friends',
      desc: 'Share your referral link and earn +2 credits per successful signup',
      color: 'text-purple-400', bg: 'bg-purple-600/10 border-purple-600/20'
    },
    {
      icon: FaBullseye,
      title: 'Use Credits',
      desc: 'Spend 1 credit per attack (max 60s). Upgrade Pro for 300s & unlimited',
      color: 'text-red-400', bg: 'bg-red-600/10 border-red-600/20'
    },
  ];
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          >
            ...
            <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1">
                ...
                {/* Pro → days left  |  Free → credit count */}
              ...
              >
                ...
                {stat.isId ? (
                  ...
                  <div>
                    ...
                    {stat.subtext && (
                      <p className={`text-[10px] mt-1 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{stat.subtext}</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            {[
              {
                to: '/attack',
                icon: FaBullseye, iconCls: 'text-red-400', iconBg: 'bg-red-600/10 border-red-600/20',
                label: 'Attack Panel',
                desc: user.isPro ? '30/day · up to 300s' : `${user.credits} credits · up to 60s`,
                accentCls: 'text-red-500/40 group-hover:text-red-400'
              },
              {
                to: '/contact',
                icon: FaCrown, iconCls: 'text-yellow-400', iconBg: 'bg-yellow-600/10 border-yellow-600/20',
                label: user.isPro ? 'Manage Plan' : 'Upgrade to Pro',
                desc: user.isPro ? 'Extend or change plan' : 'Unlimited · 300s · from ₹850',
                accentCls: 'text-yellow-500/40 group-hover:text-yellow-400'
              },
              {
                to: '/reseller-prices',
                icon: FaStore, iconCls: 'text-purple-400', iconBg: 'bg-purple-600/10 border-purple-600/20',
                label: 'Reseller Plans',
                desc: 'Bulk pricing & reseller deals',
                accentCls: 'text-purple-500/40 group-hover:text-purple-400'
              },
            ].map(item => (
              <Link
                key={item.to}
                to={item.to}
                className={`rounded-2xl p-4 border flex items-center gap-3 transition-all group hover:-translate-y-0.5 ${cardCls}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`font-bold text-sm ${dark ? 'text-white' : 'text-slate-900'}`} style={{ fontFamily: "'Rajdhani', sans-serif" }}>
                    {item.label}
                  </p>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className={`text-xs ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                  Earn <span className="text-green-400 font-bold">+2 {user.isPro ? 'bonus credits' : 'credits'}</span> per referral
                </p>
              </div>
            </div>
          ...
          {user.isPro && (user.subscriptionStatus?.daysLeft ?? 99) <= 3 && (
            ...
            >
              ...
              <div>
                <p className="font-bold text-sm text-red-400" style={{ fontFamily: "'Rajdhani', sans-serif" }}>
                  Pro Expiring Soon
                </p>
