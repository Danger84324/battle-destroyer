...
const CaptchaWidget = forwardRef(({ onVerify, onExpire, onError, theme }, ref) => {
  ...
  /* ── Load a fresh challenge from the server ── */
  const load = useCallback(async () => {
    ...
    setChallenge(null);
    try {
      const { data } = await axios.get(`${API_URL}/api/captcha/challenge`);
      setChallenge(data);
      setStatus('ready');
    } catch {
      setStatus('error');
  ...
  /* Expose reset() to parent (mirrors TurnstileWidget API) */
  ...
  const pick = async (optionIndex) => {
    ...
    try {
      ...
      setProgress(100);
      setStatus('done');
      // Emit the same shape that auth.js will validate
      onVerify?.({
  ...
  const card = `rounded-xl border transition-all duration-200 overflow-hidden ${
  ...
  return (
    <div className={card}>
      <div className="px-4 pt-3 pb-1">
        {/* Instruction */}
        <p className={`text-[11px] uppercase tracking-wider mb-1.5 font-semibold ${
        ...
        <div className="grid grid-cols-3 gap-1.5 mt-2.5 mb-3">
          {challenge?.options?.map((opt, i) => (
            <button
              ...
              className={`py-2 rounded-lg text-sm font-mono font-bold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                selected === i
