...
const resellerApiClient = axios.create({
...
resellerApiClient.interceptors.request.use(
  async (config) => {
...
// Response interceptor - decrypt response and handle CSRF token refresh
resellerApiClient.interceptors.response.use(
  async (response) => {
  ...
  async (error) => {
    ...
    if (error.response?.status === 403 && 
      ...
      try {
        ...
        return resellerApiClient(originalConfig);
      } catch (retryError) {
...
// Helper to manually refresh CSRF token if needed
resellerApiClient.refreshCSRFToken = async () => {
  csrfToken = null;
  csrfTokenPromise = null;
  return await getCSRFToken();
};

export default resellerApiClient;